...
PACKAGE=NavioElectrum
GIT_REPO=https://github.com/nav-io/navio-electrum
...
python3 -m pip install --cache-dir "$PIP_CACHE_DIR" --no-warn-script-location \
    navio-blsct || fail "Could not install navio-blsct"
...
hdiutil create -fs HFS+ -volname $PACKAGE -srcfolder dist/$PACKAGE.app dist/navio-electrum-$VERSION-unsigned.dmg || fail "Could not create .DMG"
