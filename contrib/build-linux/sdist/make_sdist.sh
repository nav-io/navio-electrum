...
(
    ...
    # setuptools normalizes the package name (e.g. navio_electrum-x.y.tar.gz);
    # detect the file it actually produced
    PKG_TARBALL="$(basename "$(ls "$PY_DISTDIR"/*.tar.gz)")"
    PKG_DIRNAME="${PKG_TARBALL%.tar.gz}"
    if ([ "$OMIT_UNCLEAN_FILES" = 1 ]); then
        FINAL_DISTNAME="NavioElectrum-sourceonly-$VERSION.tar.gz"
    else
        FINAL_DISTNAME="NavioElectrum-$VERSION.tar.gz"
    fi
    if ([ "$OMIT_UNCLEAN_FILES" = 1 ]); then
        mv "$PY_DISTDIR/$PKG_TARBALL" "$PY_DISTDIR/../$FINAL_DISTNAME"
        rmdir "$PY_DISTDIR"
    else
        mv "$PY_DISTDIR/$PKG_TARBALL" "$PY_DISTDIR/$FINAL_DISTNAME"
    fi
    ...
    find -exec touch -h -d '2000-11-11T11:11:11+00:00' {} +
    GZIP=-n tar --sort=name -czf "$FINAL_DISTNAME" "$PKG_DIRNAME/"
