...
It may be imported in another Navio Electrum wallet with the same seed."""
)

MSG_LIGHTNING_WARNING = _(
"""Navio Electrum uses static channel backups. If you lose your wallet file, you will need to request your channel to be force-closed by the remote peer in order to recover your funds. This assumes that the remote peer is reachable, and has not lost its own data."""
)
...
MSG_THIRD_PARTY_PLUGIN_WARNING = ' '.join([
    ...
    '<br/><br/>',
    _('Third-party plugins are not endorsed by Navio Electrum.'),
    _('Navio Electrum will not be responsible in case of theft, loss of funds or privacy that might result from third-party plugins.'),
    '<br/><br/>',
...
MSG_LN_EXPLAIN_SCB_BACKUPS = "".join([
    ...
    _("In the Navio Electrum mobile app, use the 'Send' button to scan this QR code."), " ",
    "\n\n",
...
MSG_TERMS_OF_USE = (
"""1. Navio Electrum is free software based on Electrum, distributed under the MIT licence by the Navio developers. Most notably, this means that the software is provided as is, and that it comes without warranty.

2. We are neither a bank nor a financial service provider. In addition, we do not store user account data, and we are not an intermediary in the interaction between our software and the Navio blockchain. Therefore, we do not have the possibility to freeze funds or to undo a fraudulent transaction.

3. We do not provide private user support. All issue resolutions are public, and take place on Github or public forums. If someone posing as 'Navio support' or 'Electrum support' proposes to help you via a private channel, this person is most likely an imposter trying to steal your coins."""
)
...
MSG_CONNECTMODE_SERVER_HELP = _(
    "Navio Electrum connects to a unique server in order to receive your transaction history. "
    "This server will learn your wallet addresses."
)
MSG_CONNECTMODE_NODES_HELP = _(
    "In addition to your history server, Navio Electrum will try to maintain connections with ~10 extra servers, in order to download block headers and find out the longest blockchain. "
...
MSG_CONNECTMODE_AUTOCONNECT_HELP = _(
    "Navio Electrum will always use a history server that is on the longest blockchain. "
    "If your current server is unresponsive or lagging, Navio Electrum will switch to another server."
)

MSG_CONNECTMODE_MANUAL_HELP = _(
    "Navio Electrum will stay with the server you selected. It will warn you if your server is lagging."
)

MSG_CONNECTMODE_ONESERVER_HELP = _(
    "Navio Electrum will stay with the server you selected, and it will not connect to additional nodes. "
